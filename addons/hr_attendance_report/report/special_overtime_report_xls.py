...
class OvertimeReportXlsx(models.AbstractModel):
    _name = 'report.hr_attendance_report.special_overtime_xlsx'
    _inherit = "report.report_xlsx.abstract"
    _description = "Special Overtime XLSX Report"
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.merge_range('E2:I3', 'Special Overtime Report', format1)
        ...
        for empl in empl_mapped:
                    ...
                    if tot_work_hours == 0:
                        ...
                        if schedule_id:
                            if tot_work_hours > schedule_id.special_ot:
                                ...
                                day_spl_overtime = schedule_id.special_ot - schedule_id.overtime
                            elif tot_work_hours > schedule_id.overtime:
                                special_ot_calc += tot_work_hours - schedule_id.overtime
                                overtime_calc += schedule_id.overtime - working_hrs
                                day_spl_overtime = tot_work_hours - schedule_id.overtime
                                ...
                                day_spl_overtime = 0
                            dec_part_daysot, whole_part_daysot = math.modf(day_spl_overtime)
                            dec_time_daysot = (dec_part_daysot / 100) * 60
                            dec_time_round_daysot = round(dec_time_daysot, 2)
                            tot_hours_daysot = whole_part_daysot + dec_time_round_daysot
                            tot_hours_round_daysot = format(tot_hours_daysot, ".2f")
                            tot_hrs_final_daysot = str(tot_hours_round_daysot).replace('.', ':')
                            sheet.write(row, col, tot_hrs_final_daysot, format4)
                elif not att_id:
                        ...
                        if line.employee_id.id == empl.id:
                            if line.checkin_day.strftime("%mm-%dd-%Y") == start_dt_uae.strftime("%mm-%dd-%Y"):
                                if line.worked_hours == 0 or line.worked_hours is None:
                                    ...
                                    diff_calc += line.diff
                                    day_spl_overtime = line.special_overtime
                                    dec_part_daysot, whole_part_daysot = math.modf(day_spl_overtime)
                                    dec_time_daysot = (dec_part_daysot / 100) * 60
                                    dec_time_round_daysot = round(dec_time_daysot, 2)
                                    tot_hours_daysot = whole_part_daysot + dec_time_round_daysot
            ...
            sheet.write(row, col, tot_work_sot_hrs, format4)
        ...
        sheet.write(3, col, 'Total SOT', format4)
